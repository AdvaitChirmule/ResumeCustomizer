...
export async function POST(req: Request){
  ...
  experiences.forEach(element => {
    texExperience += `
      \\resumeSubheading
        ...
        \\resumeItemListStart
          \\resumeItem{${element.description}}
        \\resumeItemListEnd
    `
  });

  texExperience = texExperience.replaceAll("#", "\\#")
  texExperience = texExperience.replaceAll("%", "\\%")

  texFull += texExperience
  ...
  texProject = texProject.replaceAll("#", "\\#")
  texProject = texProject.replaceAll("%", "\\%")
  ...
  await writeFile(texFilePath, texFull, "utf-8");
...
function _(value: unknown, index: number, array: unknown[]): unknown {
